...
2.  Geben Sie den folgenden Befehl in das Eingabeaufforderungsfenster ein. Die eckigen Klammern zeigen optionale Parameter an. Geben Sie den Befehl ohne die eckigen Klammern ein. Wenn Sie Windows als Betriebssystem verwenden und die Benutzerkontensteuerung aktiviert ist, sind für die Ausführung des Setups erhöhte Rechte erforderlich. Die Eingabeaufforderung muss als Administrator ausgeführt werden.  
    ...
    |[ /SAPWD=*StrongPassword* ]|Gibt das Kennwort für das [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] `sa` Konto an. Dieser Parameter ist erforderlich, wenn die Instanz den gemischten Authentifizierungsmodus ([!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] - und Windows-Authentifizierung) verwendet.<br /><br /> ** \* \* Sicherheits \* Hinweis \* ** das `sa` Konto ist ein bekanntes Konto, das [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] häufig von böswilligen Benutzern betroffen ist. Es ist sehr wichtig, dass Sie ein sicheres Kennwort für die `sa`-Anmeldung verwenden.<br /><br /> Geben Sie diesen Parameter nicht für den Windows-Authentifizierungsmodus an.|  
...
##  <a name="create-a-new-msdb-database"></a><a name="CreateMSDB"></a> Erstellen einer neuen msdb-Datenbank  
 Wenn die `msdb` Datenbank beschädigt ist und Sie nicht über eine Sicherung der- `msdb` Datenbank verfügen, können Sie mit `msdb` dem **instmsdb** -Skript eine neue erstellen.  
  
> [!WARNING]  
>  Durch das erneute Erstellen der `msdb` Datenbank mit dem **instmsdb** -Skript werden alle in gespeicherten Informationen, wie z. b. `msdb` Aufträge, Warnungen, Operatoren, Wartungspläne, Sicherungs Verlauf, Einstellungen der Richtlinien basierten Verwaltung, Datenbank-E-Mail, Leistungs Data Warehouse usw., vermieden.  
...
3.  Trennen Sie die Datenbank in einem anderen Befehlszeilenfenster `msdb` , indem Sie den folgenden Befehl ausführen und *\<servername>* dabei durch die Instanz von ersetzen [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] :`SQLCMD -E -S<servername> -dmaster -Q"EXEC sp_detach_db msdb"`  
  
4.  Benennen Sie die Datenbankdateien mithilfe von Windows-Explorer um `msdb` . Diese befinden sich standardmäßig im Unterordner DATA der [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] -Instanz.  
...
9. Erstellen Sie den in der Datenbank gespeicherten Benutzer Inhalt `msdb` , z. b. Aufträge, Warnungen usw. neu.  
