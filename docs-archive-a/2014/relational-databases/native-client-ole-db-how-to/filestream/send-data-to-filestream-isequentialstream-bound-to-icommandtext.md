...
ms.locfileid: "87616979"
---
# <a name="send-data-to-a-filestream-column-using-isequentialstream-bound-to-icommandtext-parameter-ole-db"></a><span data-ttu-id="a01b6-102">Senden von Daten an eine FILESTREAM-Spalte mithilfe einer ISequentialStream-Schnittstelle, die an einen ICommandText Parameter gebunden ist (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="a01b6-102">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter (OLE DB)</span></span>
  <span data-ttu-id="a01b6-103">Dieses Beispiel verwendet eine ISequentialStream-Schnittstelle, die an einen ICommandText-Parameter gebunden ist und dazu dient, zwischen 4 MB und 4 GB Daten an eine FILESTREAM-Spalte zu senden.</span><span class="sxs-lookup"><span data-stu-id="a01b6-103">This sample uses an ISequentialStream interface bound to an ICommandText parameter to send between 4MB and 4GB of data to a filestream column.</span></span>  
  
 <span data-ttu-id="a01b6-104">Weitere Informationen zur FILESTREAM-Funktion finden Sie [unter FILESTREAM-Unterstützung &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span><span class="sxs-lookup"><span data-stu-id="a01b6-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="a01b6-105">Beispiel</span><span class="sxs-lookup"><span data-stu-id="a01b6-105">Example</span></span>  
 <span data-ttu-id="a01b6-106">Aktivieren Sie die FILESTREAM-Unterstützung, bevor Sie dieses Beispiel kompilieren und ausführen ([Aktivieren und Konfigurieren von FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span><span class="sxs-lookup"><span data-stu-id="a01b6-106">Before you compile and run this sample, enable FILESTREAM support ([Enable and Configure FILESTREAM](../../blob/enable-and-configure-filestream.md)).</span></span>  
  
 <span data-ttu-id="a01b6-107">Stellen Sie sicher, dass die INCLUDE-Umgebungsvariable das Verzeichnis einschließt, das sqlncli.h enthält.</span><span class="sxs-lookup"><span data-stu-id="a01b6-107">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="a01b6-108">Der Server muss ein Verzeichnis mit dem Namen C:\DBFsa aufweisen; dort erstellt das Beispiel die Datenbank.</span><span class="sxs-lookup"><span data-stu-id="a01b6-108">The server must have a directory called C:\DBFsa; this is where the sample will create the database.</span></span> <span data-ttu-id="a01b6-109">Die Instanz von [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] muss Schreibzugriff auf diesen Speicherort haben (melden Sie sich z. B. als lokales Systemkonto an).</span><span class="sxs-lookup"><span data-stu-id="a01b6-109">Your instance of [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] must have write access to this location (for example, log on as a local system account).</span></span>  
  
 <span data-ttu-id="a01b6-110">Kopieren Sie das erste Codelisting, und fügen Sie es in eine Datei mit dem Namen ISSHelper.h ein.</span><span class="sxs-lookup"><span data-stu-id="a01b6-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="a01b6-111">Kopieren Sie das zweite Codelisting, und fügen Sie es in eine Datei mit dem Namen ISSHelper.cpp ein.</span><span class="sxs-lookup"><span data-stu-id="a01b6-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="a01b6-112">Kopieren Sie das dritte Codelisting, und fügen Sie es in eine Datei mit dem Namen ICommandUpload.cpp ein.</span><span class="sxs-lookup"><span data-stu-id="a01b6-112">Copy the third code listing and paste it into a file called ICommandUpload.cpp.</span></span>  
  
 <span data-ttu-id="a01b6-113">Kompilieren Sie ICommandUpload.cpp, ISSHelper.cpp, ole32.lib und oleaut32.lib.</span><span class="sxs-lookup"><span data-stu-id="a01b6-113">Compile ICommandUpload.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="a01b6-114">Wenn Sie dieses Beispiel ausführen, müssen Sie den Namen eines Servers oder Server\Instanzname sowie einen Wert zwischen 4 MB (0x400001) und 4 GB (0xFFFFFFFF) übergeben, der die Menge der zu schreibenden Daten angibt.</span><span class="sxs-lookup"><span data-stu-id="a01b6-114">When you run this sample, you must pass the name of a server, or a server\instance_name, as well as a value between 4 MB (0x400001) and 4 GB (0xFFFFFFFF) indicating the amount of data to write.</span></span>  
  
 <span data-ttu-id="a01b6-115">Das vierte Codelisting ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) löscht die in diesem Beispiel erstellte Datenbank.</span><span class="sxs-lookup"><span data-stu-id="a01b6-115">The fourth ([!INCLUDE[tsql](../../../includes/tsql-md.md)]) code listing deletes the database created by this sample.</span></span>  
