---
title: Vorgehensweisen für OLE DB | Microsoft-Dokumentation
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.technology: native-client
ms.topic: reference
helpviewer_keywords:
- OLE DB, how-to topics
ms.assetid: fbfab1b0-433d-497e-ae07-9b21a5c6903c
author: rothja
ms.author: jroth
ms.openlocfilehash: 006962c1a28cc4a21f3e2efaa63b45b0871e208f
ms.sourcegitcommit: ad4d92dce894592a259721a1571b1d8736abacdb
ms.translationtype: MT
ms.contentlocale: de-DE
ms.lasthandoff: 08/04/2020
ms.locfileid: "87616974"
---
# <a name="ole-db-how-to-topics"></a>Vorgehensweisen für OLE DB
  Wenn Sie den [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client-OLE DB-Anbieter verwenden möchten, müssen Sie wissen, wie Sie eine Verbindung mit dem Server herstellen, den Befehl ausführen und die Ergebnisse verarbeiten.  
  
## <a name="in-this-section"></a>In diesem Abschnitt  
  
-   [Vorgehensweisen zum Verarbeiten von Ergebnissen &#40;OLE DB&#41;](results/processing-results-how-to-topics-ole-db.md)  
  
-   [Festlegen großer Datenmengen &#40;OLE DB&#41;](set-large-data-ole-db.md)  
  
-   [Auflisten von OLE DB-Datenquellen &#40;OLE DB&#41;](enumerate-ole-db-data-sources-ole-db.md)  
  
-   [Massenkopieren von Daten mithilfe von IRowsetFastLoad &#40;OLE DB&#41;](../native-client-ole-db-interfaces/irowsetfastload-ole-db.md)  
  
-   [Abrufen eines FAST_FORWARD-Cursors](obtain-a-fast-forward-cursor.md)  
  
-   [Abrufen von Zeilen mithilfe von Lesezeichen &#40;OLE DB&#41;](retrieve-rows-using-bookmarks-ole-db.md)  
  
-   [Abrufen von Spalten mithilfe von IRow::GetColumns &#40;oder IRow::Open&#41; und ISequentialStream](fetch-columns-using-irow-getcolumns-or-irow-open-and-isequentialstream.md)  
  
-   [Abrufen von Spalten mithilfe von IRow::GetColumns &#40;OLE DB&#41;](fetch-columns-using-irow-getcolumns-ole-db.md)  
  
-   [Ändern eines SQL Server-Authentifizierungsbenutzerkennworts &#40;OLE DB&#41;](change-a-sql-server-authentication-user-password-ole-db.md)  
  
-   [Verwenden von erweiterten Datums- und Uhrzeitfeatures &#40;OLE DB&#41;](use-enhanced-date-and-time-features-ole-db.md)  
  
-   [FILESTREAM und OLE DB](filestream/filestream-and-ole-db.md)  
  
-   [Senden von BLOB-Daten an SQL SERVER mit IROWSETFASTLOAD und ISEQUENTIALSTREAM &#40;OLE DB&#41;](send-blob-data-to-sql-server-using-irowsetfastload-and-isequentialstream-ole-db.md)  
  
-   [Verwenden von großen CLR-UDTs &#40;OLE DB&#41;](use-large-clr-udts-ole-db.md)  
  
-   [Anzeigen von Spalten- und Katalogmetadaten für Sparsespalten &#40;OLE DB&#41;](display-column-and-catalog-metadata-for-sparse-columns-ole-db.md)  
  
-   [Integrierte Kerberos-Authentifizierung &#40;OLE DB&#41;](integrated-kerberos-authentication-ole-db.md)  
  
-   [Verwenden von Tabellenwertparametern &#40;OLE DB&#41;](../native-client-ole-db-table-valued-parameters/table-valued-parameters-ole-db.md)  
  
## <a name="see-also"></a>Weitere Informationen  
 [Programmierung für SQL Server Native Client](../native-client/sql-server-native-client-programming.md)  
  
  
