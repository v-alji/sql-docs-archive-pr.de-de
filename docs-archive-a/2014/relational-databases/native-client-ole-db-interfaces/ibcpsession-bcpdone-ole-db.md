...
ms.locfileid: "87621502"
---
# <a name="ibcpsessionbcpdone-ole-db"></a><span data-ttu-id="c7f8b-102">IBCPSession::BCPDone (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="c7f8b-102">IBCPSession::BCPDone (OLE DB)</span></span>
  <span data-ttu-id="c7f8b-103">Führt einen Commit für die übrigen Zeilen aus, die an [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]gesendet werden sollen.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-103">Commits the remaining rows to be sent to [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)].</span></span>  
  
## <a name="syntax"></a><span data-ttu-id="c7f8b-104">Syntax</span><span class="sxs-lookup"><span data-stu-id="c7f8b-104">Syntax</span></span>  
...
## <a name="remarks"></a><span data-ttu-id="c7f8b-105">Bemerkungen</span><span class="sxs-lookup"><span data-stu-id="c7f8b-105">Remarks</span></span>  
 <span data-ttu-id="c7f8b-106">Nach dem Aufruf der **BCPDone**-Methode kann kein weiterer Vorgang für die [IBCPSession](ibcpsession-ole-db.md)-Schnittstelle aufgerufen werden.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-106">No other operation can be called on the [IBCPSession](ibcpsession-ole-db.md) interface after calling the **BCPDone** method.</span></span> <span data-ttu-id="c7f8b-107">Die einzige Möglichkeit besteht darin, die [IBCPSession::BCPInit](ibcpsession-bcpinit-ole-db.md)-Methode aufzurufen, um einen neuen Massenkopiervorgang zu initiieren.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-107">The only possibility is to call the [IBCPSession::BCPInit](ibcpsession-bcpinit-ole-db.md) method to initiate a new bulk copy operation.</span></span> <span data-ttu-id="c7f8b-108">Dies gleicht dem Aufruf der [IRowsetFastLoad::Commit](irowsetfastload-commit-ole-db.md)-Methode.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-108">This is similar to calling the [IRowsetFastLoad::Commit](irowsetfastload-commit-ole-db.md) method.</span></span>  
  
## <a name="return-code-values"></a><span data-ttu-id="c7f8b-109">Rückgabecodewerte</span><span class="sxs-lookup"><span data-stu-id="c7f8b-109">Return Code Values</span></span>  
 <span data-ttu-id="c7f8b-110">S_OK</span><span class="sxs-lookup"><span data-stu-id="c7f8b-110">S_OK</span></span>  
 <span data-ttu-id="c7f8b-111">Die Methode wurde erfolgreich ausgeführt.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-111">The method succeeded.</span></span>  
  
 <span data-ttu-id="c7f8b-112">E_UNEXPECTED</span><span class="sxs-lookup"><span data-stu-id="c7f8b-112">E_UNEXPECTED</span></span>  
 <span data-ttu-id="c7f8b-113">Die Methode wurde unerwartet aufgerufen.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-113">The call to the method was unexpected.</span></span> <span data-ttu-id="c7f8b-114">Die **BCPInit** -Methode wurde beispielsweise erst nach dem Aufruf dieser Methode aufgerufen.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-114">For example, the **BCPInit** method was not called before calling this method.</span></span>  
  
## <a name="example"></a><span data-ttu-id="c7f8b-115">Beispiel</span><span class="sxs-lookup"><span data-stu-id="c7f8b-115">Example</span></span>  
 <span data-ttu-id="c7f8b-116">In diesem Beispiel wird die Verwendung der **IBCPSession** -Schnittstelle dargestellt.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-116">This sample shows how to use the **IBCPSession** interface.</span></span>  
  
 <span data-ttu-id="c7f8b-117">Das folgende [!INCLUDE[tsql](../../includes/tsql-md.md)] muss vor der Durchführung dieses Beispiels ausgeführt werden:</span><span class="sxs-lookup"><span data-stu-id="c7f8b-117">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] must be executed before running this sample:</span></span>  
...
```  
 ...
 <span data-ttu-id="c7f8b-118">In diesem Beispiel wird eine Verbindung zur master-Datenbank hergestellt.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-118">The sample connects to the master database.</span></span>  
  
 <span data-ttu-id="c7f8b-119">Bei der Ausführung des Beispiels wird outfile.dat im Projektverzeichnis erstellt.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-119">When the sample runs, it creates outfile.dat in the project directory.</span></span> <span data-ttu-id="c7f8b-120">Die Datei outfile.dat enthält die Daten, die von der Tabelle im systemeigenen (binären) Modus kopiert wurden.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-120">The outfile.dat contains the data copied from the table in native (binary) mode.</span></span>  
  
 <span data-ttu-id="c7f8b-121">Sie könnten BCP verwenden, um diese Daten mit dem folgenden Befehl wieder der Tabelle hinzuzufügen:</span><span class="sxs-lookup"><span data-stu-id="c7f8b-121">You could use BCP to add this data back into the table with the following command:</span></span>  
  
 <span data-ttu-id="c7f8b-122">**bcp master..fltest in outfile.dat -n -T -S** *server*</span><span class="sxs-lookup"><span data-stu-id="c7f8b-122">**bcp master..fltest in outfile.dat -n -T -S** *server*</span></span>  
  
 <span data-ttu-id="c7f8b-123">Sie müssen beim Kompilieren dieses Beispiels sqlncli11.lib angeben.</span><span class="sxs-lookup"><span data-stu-id="c7f8b-123">You will need to specify sqlncli11.lib when compiling this sample.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="c7f8b-124">Weitere Informationen</span><span class="sxs-lookup"><span data-stu-id="c7f8b-124">See Also</span></span>  
 <span data-ttu-id="c7f8b-125">[IBCPSession &#40;OLE DB&#41;](ibcpsession-ole-db.md) </span><span class="sxs-lookup"><span data-stu-id="c7f8b-125">[IBCPSession &#40;OLE DB&#41;](ibcpsession-ole-db.md) </span></span>  
 [<span data-ttu-id="c7f8b-126">Durchführen von Massenkopiervorgängen</span><span class="sxs-lookup"><span data-stu-id="c7f8b-126">Performing Bulk Copy Operations</span></span>](../native-client/features/performing-bulk-copy-operations.md)  
