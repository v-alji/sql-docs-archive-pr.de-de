...
ms.locfileid: "87700869"
---
# <a name="executing-an-updategram-by-using-ole-db-sqlxml-40"></a><span data-ttu-id="9d32e-102">Ausführen eines Updategrams mit OLE DB (SQLXML 4.0)</span><span class="sxs-lookup"><span data-stu-id="9d32e-102">Executing an Updategram by Using OLE DB (SQLXML 4.0)</span></span>
  <span data-ttu-id="9d32e-103">Dieses Thema enthält ein funktionierendes Beispiel für usingole DB zum Ausführen eines Update grams.</span><span class="sxs-lookup"><span data-stu-id="9d32e-103">This topic provides a working sample of usingOLE DB to execute an updategram.</span></span>  
  
## <a name="using-icommandstream-to-set-an-xml-command"></a><span data-ttu-id="9d32e-104">Verwenden von 'IcommandStream' zum Festlegen eines XML-Befehls</span><span class="sxs-lookup"><span data-stu-id="9d32e-104">Using ICommandStream to Set an XML Command</span></span>  
 <span data-ttu-id="9d32e-105">Die ICommandStream-Schnittstelle OLE DB (Version 2,6 oder höher) übergibt einen Befehl als Datenstrom Objekt und nicht als Zeichenfolge.</span><span class="sxs-lookup"><span data-stu-id="9d32e-105">The OLE DB (version 2.6 or later) ICommandStream interface passes a command as a stream object, rather than as a string.</span></span>  
  
 <span data-ttu-id="9d32e-106">Dank dieser Schnittstelle kann ein Befehl in jeder beliebigen Codierung geschrieben sein, die der XML-Parser versteht.</span><span class="sxs-lookup"><span data-stu-id="9d32e-106">This interface allows a command to be in any encoding that the XML parser understands.</span></span> <span data-ttu-id="9d32e-107">Wenn ICommand:: Execute aufgerufen wird, wird der Befehls Text direkt aus dem Stream gelesen, und es ist keine Konvertierung erforderlich.</span><span class="sxs-lookup"><span data-stu-id="9d32e-107">When ICommand::Execute is called, the command text is read out of the stream directly and no conversion is required.</span></span> <span data-ttu-id="9d32e-108">Daher ist das Ausführen von XML-Befehlen mithilfe der ICommandStream-Schnittstelle effizienter.</span><span class="sxs-lookup"><span data-stu-id="9d32e-108">Therefore, executing XML commands using ICommandStream interface is more efficient.</span></span>  
  
### <a name="setting-xml-as-a-command-using-icommandstream-and-retrieving-the-results-as-an-xml-document"></a><span data-ttu-id="9d32e-109">Festlegen von XML als Befehl mit 'IcommandStream' und Abrufen der Ergebnisse als XML-Dokument</span><span class="sxs-lookup"><span data-stu-id="9d32e-109">Setting XML as a Command Using ICommandStream and Retrieving the Results as an XML Document</span></span>  
 <span data-ttu-id="9d32e-110">Die ICommandStream-Schnittstelle kann verwendet werden, um XML-Dokumente als Befehl festzulegen, und die Ergebnisse können als XML-Dokument abgerufen werden.</span><span class="sxs-lookup"><span data-stu-id="9d32e-110">The ICommandStream interface can be used to set XML documents as a command, and the results can be retrieved as an XML document.</span></span>  
  
#### <a name="executing-templates-with-xpath-queries"></a><span data-ttu-id="9d32e-111">Ausführen von Vorlagen mit XPath-Abfragen</span><span class="sxs-lookup"><span data-stu-id="9d32e-111">Executing Templates with XPath Queries</span></span>  
 <span data-ttu-id="9d32e-112">Die folgende XML-Vorlage, die aus einer XPath-Abfrage besteht, wird mithilfe von ICommandStream als Befehl angegeben:</span><span class="sxs-lookup"><span data-stu-id="9d32e-112">The following XML template consisting of an XPath query is specified as a command using ICommandStream:</span></span>  
...
```  
 ...
 <span data-ttu-id="9d32e-113">Die XPath-Abfrage in der Vorlage wird für das folgende Zuordnungsschema ausgeführt:</span><span class="sxs-lookup"><span data-stu-id="9d32e-113">The XPath query in the template is executed against the following mapping schema:</span></span>  
...
```  
 ...
 <span data-ttu-id="9d32e-114">Die Abfrage gibt alle Mitarbeiterelemente zurück.</span><span class="sxs-lookup"><span data-stu-id="9d32e-114">The query returns all of the employee elements.</span></span> <span data-ttu-id="9d32e-115">Bei der Standard Zuordnung wird das- **\<Person.Contact>** Element der Person. Contact-Tabelle in der AdventureWorks-Datenbank zugeordnet.</span><span class="sxs-lookup"><span data-stu-id="9d32e-115">With default mapping, the **\<Person.Contact>** element maps to the Person.Contact table in the AdventureWorks database.</span></span>  
  
###### <a name="to-set-xml-as-a-command-and-retrieving-result-as-an-xml-document"></a><span data-ttu-id="9d32e-116">So legen Sie XML als Befehl fest und rufen das Ergebnis als XML-Dokument ab</span><span class="sxs-lookup"><span data-stu-id="9d32e-116">To set XML as a command and retrieving result as an XML document</span></span>  
  
1.  <span data-ttu-id="9d32e-117">Initialisieren Sie die Datenbank, und stellen Sie eine Verbindung zur Datenbank her.</span><span class="sxs-lookup"><span data-stu-id="9d32e-117">Initialize and establish a connection to the database.</span></span>  
  
2.  <span data-ttu-id="9d32e-118">Rufen Sie die ICommandStream-Schnittstelle für ICommand ab.</span><span class="sxs-lookup"><span data-stu-id="9d32e-118">Obtain the ICommandStream interface on ICommand.</span></span>  
  
3.  <span data-ttu-id="9d32e-119">Legen Sie die erforderlichen Eigenschaften für den Befehl fest.</span><span class="sxs-lookup"><span data-stu-id="9d32e-119">Set the necessary command properties.</span></span> <span data-ttu-id="9d32e-120">In diesem Beispiel wird die anbieterspezifische Eigenschaft SSPROP_STREAM_BASEPATH auf das Verzeichnis festgelegt, in dem das Zuordnungsschema und die Vorlagendateien gespeichert sind.</span><span class="sxs-lookup"><span data-stu-id="9d32e-120">In this example, the provider-specific property SSPROP_STREAM_BASEPATH is set to the directory where the mapping schema and the template files are stored.</span></span>  
  
4.  <span data-ttu-id="9d32e-121">Verwenden Sie ICommandStream:: SetCommandStream, um den Befehlsdaten Strom anzugeben.</span><span class="sxs-lookup"><span data-stu-id="9d32e-121">Use ICommandStream::SetCommandStream to specify the command stream.</span></span> <span data-ttu-id="9d32e-122">In diesem Beispiel wird die XML-Vorlage, die ausgeführt wird, aus einer Datei gelesen.</span><span class="sxs-lookup"><span data-stu-id="9d32e-122">In this example, the XML template being executed is read from a file.</span></span> <span data-ttu-id="9d32e-123">Dies ist nützlich, wenn Sie große XML-Vorlagen ausführen möchten.</span><span class="sxs-lookup"><span data-stu-id="9d32e-123">This is useful when you want to execute large XML templates.</span></span>  
  
5.  <span data-ttu-id="9d32e-124">Führen Sie den XML-Befehl mit ICommand:: Execute aus, und fordern Sie die IID_ISequentialStream Schnittstellen-ID an.</span><span class="sxs-lookup"><span data-stu-id="9d32e-124">Execute the XML command using ICommand::Execute, requesting the IID_ISequentialStream interface ID.</span></span>  
  
6.  <span data-ttu-id="9d32e-125">Verarbeiten Sie das Ergebnis.</span><span class="sxs-lookup"><span data-stu-id="9d32e-125">Process the result.</span></span> <span data-ttu-id="9d32e-126">In diesem Beispiel wird das aus dem Datenstrom gelesene XML auf dem Bildschirm angezeigt.</span><span class="sxs-lookup"><span data-stu-id="9d32e-126">In this example, the XML read from the stream is displayed on the screen.</span></span>  
...
#### <a name="passing-parameters-to-templates"></a><span data-ttu-id="9d32e-127">Übergeben von Parametern an Vorlagen</span><span class="sxs-lookup"><span data-stu-id="9d32e-127">Passing Parameters to Templates</span></span>  
 <span data-ttu-id="9d32e-128">In diesem Beispiel wird gezeigt, wie Parameterwerte an XML-Befehle übergeben werden können.</span><span class="sxs-lookup"><span data-stu-id="9d32e-128">This example shows how parameter values can be passed to XML commands.</span></span> <span data-ttu-id="9d32e-129">Diese XML-Vorlage wird als Befehl angegeben:</span><span class="sxs-lookup"><span data-stu-id="9d32e-129">This XML template is specified as a command:</span></span>  
...
```  
 ...
 <span data-ttu-id="9d32e-130">Die Vorlage schließt eine SQL-Abfrage ein.</span><span class="sxs-lookup"><span data-stu-id="9d32e-130">The template includes an SQL query.</span></span> <span data-ttu-id="9d32e-131">Die Abfrage erfordert einen Wert für den Parameter ( @Title ).</span><span class="sxs-lookup"><span data-stu-id="9d32e-131">The query requires a value for its parameter (@Title).</span></span> <span data-ttu-id="9d32e-132">Wenn kein Parameterwert übergeben wird, wird der Standardwert ("Mr.") verwendet.</span><span class="sxs-lookup"><span data-stu-id="9d32e-132">If no parameter value is passed, the default value ("Mr.") is used.</span></span>  
  
 <span data-ttu-id="9d32e-133">Beim Übergeben von Parameterwerten an eine Vorlage müssen sowohl der Parametername als auch der Parameterwert angegeben werden.</span><span class="sxs-lookup"><span data-stu-id="9d32e-133">In passing parameter values to a template, the parameter name and value both must be specified.</span></span>  
  
 <span data-ttu-id="9d32e-134">Der Code lautet wie folgt:</span><span class="sxs-lookup"><span data-stu-id="9d32e-134">This is the code:</span></span>  
