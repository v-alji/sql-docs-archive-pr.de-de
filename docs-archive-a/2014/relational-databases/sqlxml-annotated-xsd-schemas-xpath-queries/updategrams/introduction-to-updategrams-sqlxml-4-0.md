...
## <a name="required-namespaces-in-the-updategram"></a>Erforderliche Namespaces im Updategram  
 Die Schlüsselwörter in einem Update Gram, z **\<sync>** . b **\<before>** ., und **\<after>** , sind im- `urn:schemas-microsoft-com:xml-updategram` Namespace vorhanden. Sie können ein beliebiges Namespacepräfix verwenden. In dieser Dokumentation gibt das Präfix `updg` den Namespace `updategram` an.  
