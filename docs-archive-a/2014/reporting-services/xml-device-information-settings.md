...
|**Codierung**|Der Internet Assigned Numbers Authority (IANA)-Name einer Zeichencodierung, die von .NET Framework unterstützt wird. Standardwert: `UTF-8`. Beispiele für andere Werte: ASCII, UTF-7 und UTF-16.|  
|**File extension**|Die Dateierweiterung für die generierte Datei.|  
|**Schema**|Gibt an, ob die XML-Schemadefinition (XSD) gerendert wird oder ob die tatsächlichen XML-Daten gerendert werden. Der Wert `true` gibt an, dass ein XML-Schema gerendert wird. Standardwert: `false`.|  
