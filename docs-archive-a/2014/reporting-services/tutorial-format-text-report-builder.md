...
9. Klicken Sie mit der rechten Maustaste auf das Textfeld mit `[LinkText]` , und klicken Sie dann auf **Zellen teilen**.  
  
10. Wählen Sie die leere Zelle unterhalb der `[LinkText]` Zelle aus, halten Sie die UMSCHALTTASTE gedrückt, und wählen Sie die zwei Zellen rechts aus: die **gesamte** Zelle in der **Product** -Spalte und die- `[Sum(Sales)]` Zelle in der Spalte **Sales** .  
