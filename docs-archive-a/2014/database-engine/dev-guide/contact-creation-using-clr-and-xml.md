...
3.  Erstellen Sie in c:\MySample die Datei `Contacts.vb` (für das Visual Basic-Beispiel) oder `Contacts.cs` (für das C#-Beispiel), und kopieren Sie den entsprechenden Visual Basic- oder C#-Beispielcode (unten) in die Datei.  
...
```  
 ...
 Im folgenden [!INCLUDE[tsql](../../includes/tsql-md.md)]-Code werden die Assembly, die gespeicherte Prozedur und die eingefügten Daten aus der Datenbank entfernt.  
