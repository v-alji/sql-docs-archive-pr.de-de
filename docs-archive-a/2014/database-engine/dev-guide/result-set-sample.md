---
title: Resultsetbeispiel | Microsoft-Dokumentation
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87622868"
---
# <a name="result-set-sample"></a><span data-ttu-id="dc44f-102">Beispiel für einen Ergebnissatz</span><span class="sxs-lookup"><span data-stu-id="dc44f-102">Result Set Sample</span></span>
  <span data-ttu-id="dc44f-103">Manchmal kann es nützlich sein, Befehle beim Durchsehen der Ergebnisse einer Abfrage ausführen zu können, ohne eine neue Verbindung öffnen und ohne alle Ergebnisse in den Speicher laden zu müssen.</span><span class="sxs-lookup"><span data-stu-id="dc44f-103">Sometimes it is useful to be able to execute commands while reading through results of a query without opening a new connection and without reading all the results into memory.</span></span> <span data-ttu-id="dc44f-104">Dies kann mit der Technologie der MARS-Funktion (Multiple Active Result Set) in ADO.NET 2.0 verwirklicht werden.</span><span class="sxs-lookup"><span data-stu-id="dc44f-104">The Multiple Active Result Set (MARS) feature in ADO.NET 2.0 is a technology that can help you achieve that.</span></span> <span data-ttu-id="dc44f-105">Derzeit ist MARS nicht für den Anbieter im Prozess, der für die serverseitige Programmierung verwendet wird, implementiert.</span><span class="sxs-lookup"><span data-stu-id="dc44f-105">Currently, MARS is not implemented for the in-process provider used for server-side programming.</span></span> <span data-ttu-id="dc44f-106">Diese Einschränkung können Sie mithilfe eines serverseitigen Cursors umgehen.</span><span class="sxs-lookup"><span data-stu-id="dc44f-106">To work around this limitation, you can use server-side cursors.</span></span> <span data-ttu-id="dc44f-107">Dieses Beispiel zeigt, wie Sie serverseitige Cursor verwenden, um die fehlende MARS-Unterstützung für die serverseitige Progammierung zu umgehen.</span><span class="sxs-lookup"><span data-stu-id="dc44f-107">This sample demonstrates how to use server-side cursors to work around the lack of MARS support for server-side programming.</span></span>  
  
> [!CAUTION]  
>  <span data-ttu-id="dc44f-108">Das Verwenden serverseitiger Cursor ist, was die Serverressourcen betrifft, ein aufwendiges Verfahren und kann den Abfrageoptimierer in Microsoft SQL Server daran hindern, die Leistung Ihrer Abfragen zu verbessern.</span><span class="sxs-lookup"><span data-stu-id="dc44f-108">Using server-side cursors is expensive in terms of server resources and can sometimes prevent the query optimizer in Microsoft SQL Server from enhancing the performance of your queries.</span></span> <span data-ttu-id="dc44f-109">Deshalb sollten Sie ggf. den Code so umschreiben, dass er nach Möglichkeit immer `JOINs` verwendet.</span><span class="sxs-lookup"><span data-stu-id="dc44f-109">Therefore, you might consider rewriting your code to use `JOINs` whenever possible.</span></span>  
  
 <span data-ttu-id="dc44f-110">Die APIs für diese Klasse ähneln einem Datenleser, wobei Sie jedoch vorwärts oder rückwärts im Resultset navigieren und andere Befehle auf der Verbindung ausgeben können, wenn das Resultset geöffnet ist. Diese Implementierung ist stark vereinfacht, damit Sie das Beispiel leichter nachvollziehen können.</span><span class="sxs-lookup"><span data-stu-id="dc44f-110">The API for this class is similar to a data reader except that you can move forward or backward throughout the result set, and other commands can be issued on the connection while the result set is open.This implementation is highly simplified to make the sample easy to understand.</span></span> <span data-ttu-id="dc44f-111">Eine effizientere Implementierung würde das Abrufen mehrerer Zeilen gleichzeitig unterstützen, um eine Verarbeitung der gesamten Datenbank pro abgerufener Zeile zu verhindern.</span><span class="sxs-lookup"><span data-stu-id="dc44f-111">A more efficient implementation would fetch multiple rows to avoid a database turnaround per row fetched.</span></span> <span data-ttu-id="dc44f-112">Durch das Verwenden dieser Klasse ist der Speicherbedarf möglicherweise weitaus geringer als beim Auffüllen eines Datasets mit allen Ergebnissen einer Abfrage. Dies ist für die serverseitige Programmierung ein wichtiger Faktor.</span><span class="sxs-lookup"><span data-stu-id="dc44f-112">Using this class can have a significantly smaller memory footprint than filling a dataset with all the results of a query, which is very important for server-side programming.</span></span> <span data-ttu-id="dc44f-113">In diesem Beispiel wird veranschaulicht, wie mithilfe des Attributs "Allow partially trusted callers" angegeben wird, dass es sich bei der Resultset-Assembly um eine Bibliothek handelt, die von anderen Assemblys auf sichere Weise aufgerufen werden kann.</span><span class="sxs-lookup"><span data-stu-id="dc44f-113">This sample also demonstrates using the "Allow partially trusted callers" attribute to indicate that the Result Set assembly is a library which may be safely called from other assemblies.</span></span> <span data-ttu-id="dc44f-114">Diese Vorgehensweise ist etwas komplexer, jedoch sicherer als das Registrieren der aufrufenden Assembly mithilfe der UNSAFE-Berechtigung.</span><span class="sxs-lookup"><span data-stu-id="dc44f-114">This approach is a little more complex but much more secure than registering the calling assembly using the unsafe permission.</span></span> <span data-ttu-id="dc44f-115">Sie ist sicherer, weil durch das Registrieren der aufrufenden Assembly als SAFE die aufrufende Assembly lediglich auf für den Server externe Ressourcen zugreift und Beschädigungen der Integrität des Servers verhindert werden.</span><span class="sxs-lookup"><span data-stu-id="dc44f-115">It is more secure because registering the calling assembly as safe the calling assembly limits affecting resources off the server and prevents damage to the integrity of the server.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="dc44f-116">Voraussetzungen</span><span class="sxs-lookup"><span data-stu-id="dc44f-116">Prerequisites</span></span>  
 <span data-ttu-id="dc44f-117">Zum Erstellen und Ausführen dieses Projekts muss die folgende Software installiert sein:</span><span class="sxs-lookup"><span data-stu-id="dc44f-117">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="dc44f-118">oder [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="dc44f-118">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="dc44f-119">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express erhalten Sie kostenlos auf der [Website](https://www.microsoft.com/sql-server/sql-server-editions-express) mit der Dokumentation und den Beispielen für [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="dc44f-119">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="dc44f-120">Die AdventureWorks-Datenbank, die auf der [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer-[Website](https://go.microsoft.com/fwlink/?linkid=62796) zur Verfügung gestellt wird.</span><span class="sxs-lookup"><span data-stu-id="dc44f-120">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="dc44f-121">.NET Framework SDK 2.0 oder höher oder Microsoft Visual Studio 2005 oder höher.</span><span class="sxs-lookup"><span data-stu-id="dc44f-121">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="dc44f-122">Das .NET Framework SDK ist kostenlos erhältlich.</span><span class="sxs-lookup"><span data-stu-id="dc44f-122">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="dc44f-123">Außerdem müssen die folgenden Bedingungen erfüllt sein:</span><span class="sxs-lookup"><span data-stu-id="dc44f-123">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="dc44f-124">In der von Ihnen verwendeten [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]-Instanz muss die CLR-Integration aktiviert sein.</span><span class="sxs-lookup"><span data-stu-id="dc44f-124">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="dc44f-125">Führen Sie zum Aktivieren der CLR-Integration die folgenden Schritte aus:</span><span class="sxs-lookup"><span data-stu-id="dc44f-125">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="dc44f-126">Aktivieren der CLR-Integration</span><span class="sxs-lookup"><span data-stu-id="dc44f-126">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="dc44f-127">Führen Sie die folgenden [!INCLUDE[tsql](../../includes/tsql-md.md)]-Befehle aus:</span><span class="sxs-lookup"><span data-stu-id="dc44f-127">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="dc44f-128">Um CLR zu aktivieren, benötigen Sie die `ALTER SETTINGS`-Serverberechtigung, die Mitglieder der festen Serverrollen `sysadmin` und `serveradmin` implizit erhalten.</span><span class="sxs-lookup"><span data-stu-id="dc44f-128">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="dc44f-129">Die AdventureWorks-Datenbank muss in der von Ihnen verwendeten [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]-Instanz installiert sein.</span><span class="sxs-lookup"><span data-stu-id="dc44f-129">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="dc44f-130">Wenn Sie keine Administratorrechte für die von Ihnen verwendete [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]-Instanz besitzen, müssen Sie sich von einem Administrator die **CreateAssembly**-Berechtigung erteilen lassen, damit Sie die Installation ausführen können.</span><span class="sxs-lookup"><span data-stu-id="dc44f-130">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="dc44f-131">Erstellen des Beispiels</span><span class="sxs-lookup"><span data-stu-id="dc44f-131">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="dc44f-132">Verwenden Sie die folgenden Anweisungen, um das Beispiel zu erstellen und auszuführen:</span><span class="sxs-lookup"><span data-stu-id="dc44f-132">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="dc44f-133">Wechseln Sie zu einer Visual Studio- oder .NET Framework-Eingabeaufforderung.</span><span class="sxs-lookup"><span data-stu-id="dc44f-133">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="dc44f-134">Erstellen Sie ggf. ein Verzeichnis für das Beispiel.</span><span class="sxs-lookup"><span data-stu-id="dc44f-134">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="dc44f-135">Für dieses Beispiel wird C:\MySample verwendet.</span><span class="sxs-lookup"><span data-stu-id="dc44f-135">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="dc44f-136">Erstellen Sie eine Schlüsseldatei mit starkem Namen, indem Sie an der Eingabeaufforderung Folgendes ausführen.</span><span class="sxs-lookup"><span data-stu-id="dc44f-136">Create a strong  name key file by executing the following at the command prompt.</span></span>  
  
## <a name="sample-code"></a><span data-ttu-id="dc44f-137">Beispielcode</span><span class="sxs-lookup"><span data-stu-id="dc44f-137">Sample Code</span></span>  
 <span data-ttu-id="dc44f-138">Die Codelistings für dieses Beispiel lauten wie folgt.</span><span class="sxs-lookup"><span data-stu-id="dc44f-138">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="dc44f-139">Dies ist der Code für das Bibliotheks-`ResultSet.`</span><span class="sxs-lookup"><span data-stu-id="dc44f-139">This is the code for the Library `ResultSet.`</span></span>  
  
 <span data-ttu-id="dc44f-140">C#</span><span class="sxs-lookup"><span data-stu-id="dc44f-140">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="dc44f-141">Dies ist der Code zum Testen des Bibliotheks-`ResultSet.`</span><span class="sxs-lookup"><span data-stu-id="dc44f-141">This is the code to test the library `ResultSet.`</span></span>  
  
 <span data-ttu-id="dc44f-142">C#</span><span class="sxs-lookup"><span data-stu-id="dc44f-142">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="dc44f-143">Dies ist das [!INCLUDE[tsql](../../includes/tsql-md.md)]-Installationsskript (`Install.sql`), das die Assemblys bereitstellt und die gespeicherten Prozeduren erstellt.</span><span class="sxs-lookup"><span data-stu-id="dc44f-143">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assemblies and creates the stored procedures.</span></span>  
...
```  
 ...
 <span data-ttu-id="dc44f-144">Dies ist das [!INCLUDE[tsql](../../includes/tsql-md.md)]-Testskript (`test.sql`), mit dem das Beispiel getestet wird.</span><span class="sxs-lookup"><span data-stu-id="dc44f-144">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] test script (`test.sql`), which tests the sample.</span></span>  
...
```  
 ...
 <span data-ttu-id="dc44f-145">Im folgenden [!INCLUDE[tsql](../../includes/tsql-md.md)]-Code werden die Assemblys und die gespeicherten Prozeduren aus der Datenbank entfernt.</span><span class="sxs-lookup"><span data-stu-id="dc44f-145">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assemblies and stored procedures from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="dc44f-146">Weitere Informationen</span><span class="sxs-lookup"><span data-stu-id="dc44f-146">See Also</span></span>  
 [<span data-ttu-id="dc44f-147">Verwendungsszenarios und Beispiele für Common Language Runtime-Integration &#40;CLR&#41;</span><span class="sxs-lookup"><span data-stu-id="dc44f-147">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
