...
3.  Erstellen Sie eine Schlüsseldatei mit starkem Namen, indem Sie an der Eingabeaufforderung Folgendes ausführen.  
...
```  
 ...
 Dies ist der Code zum Testen des Bibliotheks-`ResultSet.`  
...
```  
 ...
 Dies ist das [!INCLUDE[tsql](../../includes/tsql-md.md)]-Installationsskript (`Install.sql`), das die Assemblys bereitstellt und die gespeicherten Prozeduren erstellt.  
...
```  
 ...
 Im folgenden [!INCLUDE[tsql](../../includes/tsql-md.md)]-Code werden die Assemblys und die gespeicherten Prozeduren aus der Datenbank entfernt.  
