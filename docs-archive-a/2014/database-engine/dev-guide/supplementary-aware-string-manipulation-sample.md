---
title: Beispiel für eine ergänzende Zeichen folgen Bearbeitung | Microsoft-Dokumentation
ms.custom: ''
ms.date: 06/13/2017
...
ms.locfileid: "87701721"
---
# <a name="supplementary-aware-string-manipulation-sample"></a><span data-ttu-id="cbffc-102">Beispiel für Zeichenfolgendarstellung mit ergänzenden Zeichen</span><span class="sxs-lookup"><span data-stu-id="cbffc-102">Supplementary-Aware String Manipulation Sample</span></span>
  <span data-ttu-id="cbffc-103">Dieses Beispiel für [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] veranschaulicht Zeichenfolgenbearbeitungen mit ergänzenden Zeichen.</span><span class="sxs-lookup"><span data-stu-id="cbffc-103">This sample for [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] demonstrates supplementary character aware string processing.</span></span> <span data-ttu-id="cbffc-104">Dieses Beispiel zeigt die Implementierung von fünf Transact-SQL-Zeichenfolgenfunktionen, die dieselben Bearbeitungsfunktionen für Zeichenfolgen bereitstellen wie die integrierten Funktionen, jedoch zusätzliche Funktionen für ergänzende Zeichen bieten, um sowohl Unicode- als auch Sonderzeichenfolgen bearbeiten zu können.</span><span class="sxs-lookup"><span data-stu-id="cbffc-104">This sample shows the implementation of five Transact-SQL string functions that provide the same string manipulation functions as the built-in functions, but with additional supplementary character-aware capability to handle both Unicode and supplementary character strings.</span></span> <span data-ttu-id="cbffc-105">Die fünf Funktionen sind "Lens ()" `lefts(), rights(), subs()` und die `replace_s()` den integrierten Funktionen `LEN(), LEFT(), RIGHT(), SUBSTRING()` und Zeichen folgen Funktionen entsprechen `REPLACE()` .</span><span class="sxs-lookup"><span data-stu-id="cbffc-105">The five functions are lens(), `lefts(), rights(), subs()` and `replace_s()` which are equivalent to the built-in functions `LEN(), LEFT(), RIGHT(), SUBSTRING()` and `REPLACE()` string functions.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="cbffc-106">Voraussetzungen</span><span class="sxs-lookup"><span data-stu-id="cbffc-106">Prerequisites</span></span>  
 <span data-ttu-id="cbffc-107">Zum Erstellen und Ausführen dieses Projekts muss die folgende Software installiert sein:</span><span class="sxs-lookup"><span data-stu-id="cbffc-107">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="cbffc-108">oder [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="cbffc-108">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="cbffc-109">[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express erhalten Sie kostenlos auf der [Website](https://www.microsoft.com/sql-server/sql-server-editions-express) mit der Dokumentation und den Beispielen für [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span><span class="sxs-lookup"><span data-stu-id="cbffc-109">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="cbffc-110">Die AdventureWorks-Datenbank, die auf der [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer-[Website](https://go.microsoft.com/fwlink/?linkid=62796) zur Verfügung gestellt wird.</span><span class="sxs-lookup"><span data-stu-id="cbffc-110">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="cbffc-111">.NET Framework SDK 2.0 oder höher oder Microsoft Visual Studio 2005 oder höher.</span><span class="sxs-lookup"><span data-stu-id="cbffc-111">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="cbffc-112">Das .NET Framework SDK ist kostenlos erhältlich.</span><span class="sxs-lookup"><span data-stu-id="cbffc-112">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="cbffc-113">Außerdem müssen die folgenden Bedingungen erfüllt sein:</span><span class="sxs-lookup"><span data-stu-id="cbffc-113">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="cbffc-114">In der von Ihnen verwendeten [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]-Instanz muss die CLR-Integration aktiviert sein.</span><span class="sxs-lookup"><span data-stu-id="cbffc-114">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="cbffc-115">Führen Sie zum Aktivieren der CLR-Integration die folgenden Schritte aus:</span><span class="sxs-lookup"><span data-stu-id="cbffc-115">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="cbffc-116">Aktivieren der CLR-Integration</span><span class="sxs-lookup"><span data-stu-id="cbffc-116">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="cbffc-117">Führen Sie die folgenden [!INCLUDE[tsql](../../includes/tsql-md.md)]-Befehle aus:</span><span class="sxs-lookup"><span data-stu-id="cbffc-117">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="cbffc-118">Um CLR zu aktivieren, benötigen Sie die `ALTER SETTINGS`-Serverberechtigung, die Mitglieder der festen Serverrollen `sysadmin` und `serveradmin` implizit erhalten.</span><span class="sxs-lookup"><span data-stu-id="cbffc-118">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="cbffc-119">Die AdventureWorks-Datenbank muss in der von Ihnen verwendeten [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]-Instanz installiert sein.</span><span class="sxs-lookup"><span data-stu-id="cbffc-119">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="cbffc-120">Wenn Sie keine Administratorrechte für die von Ihnen verwendete [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]-Instanz besitzen, müssen Sie sich von einem Administrator die **CreateAssembly**-Berechtigung erteilen lassen, damit Sie die Installation ausführen können.</span><span class="sxs-lookup"><span data-stu-id="cbffc-120">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="cbffc-121">Erstellen des Beispiels</span><span class="sxs-lookup"><span data-stu-id="cbffc-121">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="cbffc-122">Verwenden Sie die folgenden Anweisungen, um das Beispiel zu erstellen und auszuführen:</span><span class="sxs-lookup"><span data-stu-id="cbffc-122">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="cbffc-123">Wechseln Sie zu einer Visual Studio- oder .NET Framework-Eingabeaufforderung.</span><span class="sxs-lookup"><span data-stu-id="cbffc-123">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="cbffc-124">Erstellen Sie ggf. ein Verzeichnis für das Beispiel.</span><span class="sxs-lookup"><span data-stu-id="cbffc-124">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="cbffc-125">Für dieses Beispiel wird C:\MySample verwendet.</span><span class="sxs-lookup"><span data-stu-id="cbffc-125">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="cbffc-126">Da dieses Beispiel eine signierte Assembly erfordert, erstellen Sie einen asymmetrischen Schlüssel, indem Sie folgenden Befehl eingeben:</span><span class="sxs-lookup"><span data-stu-id="cbffc-126">Since this example requires a signed assembly, create an asymmetric key by typing the command:</span></span>  
  
     `sn -k SampleKey.snk`  
  
4.  <span data-ttu-id="cbffc-127">Kompilieren Sie an der Eingabeaufforderung den Beispielcode, indem Sie je nach gewählter Sprache eine der folgenden Anweisungen ausführen.</span><span class="sxs-lookup"><span data-stu-id="cbffc-127">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
5.  <span data-ttu-id="cbffc-128">Kopieren Sie den [!INCLUDE[tsql](../../includes/tsql-md.md)]-Installationscode in eine Datei, und speichern Sie sie als `Install.sql` im Beispielverzeichnis.</span><span class="sxs-lookup"><span data-stu-id="cbffc-128">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
6.  <span data-ttu-id="cbffc-129">Stellen Sie die Assembly und die gespeicherte Prozedur bereit, indem Sie die folgende Anweisung ausführen:</span><span class="sxs-lookup"><span data-stu-id="cbffc-129">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql -v root = "C:\MySample\"`  
  
7.  <span data-ttu-id="cbffc-130">Kopieren [!INCLUDE[tsql](../../includes/tsql-md.md)] Sie das Test Befehls Skript in eine Datei, und speichern Sie Sie als `test.sql` im Beispiel Verzeichnis.</span><span class="sxs-lookup"><span data-stu-id="cbffc-130">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
8.  <span data-ttu-id="cbffc-131">Führen Sie das Testskript mit dem folgenden Befehl aus:</span><span class="sxs-lookup"><span data-stu-id="cbffc-131">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
9. <span data-ttu-id="cbffc-132">Kopieren Sie das [!INCLUDE[tsql](../../includes/tsql-md.md)]-Bereinigungsskript in eine Datei, und speichern Sie diese als `cleanup.sql` im Beispielverzeichnis.</span><span class="sxs-lookup"><span data-stu-id="cbffc-132">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
10. <span data-ttu-id="cbffc-133">Führen Sie das Skript mit dem folgenden Befehl aus:</span><span class="sxs-lookup"><span data-stu-id="cbffc-133">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="cbffc-134">Beispielcode</span><span class="sxs-lookup"><span data-stu-id="cbffc-134">Sample Code</span></span>  
 <span data-ttu-id="cbffc-135">Die Codelistings für dieses Beispiel lauten wie folgt.</span><span class="sxs-lookup"><span data-stu-id="cbffc-135">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="cbffc-136">C#</span><span class="sxs-lookup"><span data-stu-id="cbffc-136">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="cbffc-137">Visual Basic</span><span class="sxs-lookup"><span data-stu-id="cbffc-137">Visual Basic</span></span>  
...
```  
 ...
 <span data-ttu-id="cbffc-138">Dies ist das [!INCLUDE[tsql](../../includes/tsql-md.md)]-Installationsskript (`Install.sql`), das die Assembly bereitstellt und die Funktionen mit ergänzenden Funktionen in der Datenbank erstellt.</span><span class="sxs-lookup"><span data-stu-id="cbffc-138">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the supplementary-aware functions in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="cbffc-139">Dies ist die Datei `test.sql`, die das Beispiel durch das Ausführen der Funktionen testet.</span><span class="sxs-lookup"><span data-stu-id="cbffc-139">This is `test.sql`, which tests the sample by executing the functions.</span></span>  
...
```  
 ...
 <span data-ttu-id="cbffc-140">Im folgenden [!INCLUDE[tsql](../../includes/tsql-md.md)]-Code werden die Assembly und die Funktionen aus der Datenbank entfernt.</span><span class="sxs-lookup"><span data-stu-id="cbffc-140">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and functions from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="cbffc-141">Weitere Informationen</span><span class="sxs-lookup"><span data-stu-id="cbffc-141">See Also</span></span>  
 [<span data-ttu-id="cbffc-142">Verwendungsszenarios und Beispiele für Common Language Runtime-Integration &#40;CLR&#41;</span><span class="sxs-lookup"><span data-stu-id="cbffc-142">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
