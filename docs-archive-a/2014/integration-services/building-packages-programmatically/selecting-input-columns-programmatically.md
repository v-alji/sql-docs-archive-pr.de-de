...
## <a name="selecting-columns"></a>Auswählen von Spalten  
 Rufen Sie die <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSInput100.GetVirtualInput%2A>-Methode auf, um die Liste verfügbarer Spalten von einer Upstreamkomponente abzurufen. Rufen Sie anschließend die <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.CManagedComponentWrapperClass.SetUsageType%2A>-Methode der Entwurfszeit-Komponenteninstanz auf, um Spalten aus der virtuellen Eingabespaltenauflistung auszuwählen. Wenn Sie diese Methode aufrufen, wird durch die Komponente eine neue Eingabespalte in ihrer Eingabespaltenauflistung erstellt, die die gleiche Herkunfts-ID wie die entsprechende Spalte in der Ausgabeauflistung der Upstreamkomponente aufweist.  
  
 Wählen Sie keine Spalten aus, indem Sie die <xref:Microsoft.SqlServer.Dts.Pipeline.Wrapper.IDTSVirtualInput100.SetUsageType%2A>-Methode des virtuellen Eingabeobjekts direkt aufrufen, da in diesem Fall die Komponente Spalten basierend auf nicht passenden Datentypen oder anderen Eigenschaften nicht mehr ablehnen kann.  
  
## <a name="sample"></a>Beispiel  
 Im folgenden Codebeispiel wird veranschaulicht, wie die Entwurfszeitinstanz einer Komponente verwendet wird, um die Spalten für eine Komponente auszuwählen.  
